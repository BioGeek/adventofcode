...
def main(part=1, data=None):
    ...
    dicts = []
    for line in data:
        m = re.match('\[1518-(?P<month>\d{2})-(?P<day>\d{2}) (?P<hour>\d{2}):(?P<mins>\d{2})\] (?P<line>.*)', line)
        if m:
    ...
    if part == 1:
        ...
        most_overlapping_minute = max(overlapping[guard_most_asleep], key=lambda k: overlapping[guard_most_asleep][k])
